...
$stmt = $pdo->query("SELECT id, nume, email, grupa, poza, created_at FROM studenti ORDER BY created_at DESC");
            ...
            <tbody>
                <?php foreach ($studenti as $s): ?>
                    <tr>
						...
						<td><?php if (!empty($s['poza'])): ?>
							<img src="uploads/<?php echo htmlspecialchars($s['poza'], ENT_QUOTES, 'UTF-8'); ?>"
							alt="poza" style="width:48px;height:48px;object-fit:cover;border-radius:10px;">
							<?php else: ?>-<?php endif; ?></td>
					    <td><a class="btn" href="editeaza.php?id=<?php echo (int)$s['id']; ?>">Editează</a>
						</td>
